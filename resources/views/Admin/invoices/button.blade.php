...
<div class="dt-buttons flex-wrap">
    ...
    <a class="btn btn-light-primary me-3"  href="{{url('invoices/add/'.'outcome')}}" >
        {{__('lang.add_invoice')}}
        <i class="bi bi-plus-circle-fill fs-2x"></i>
    </a>
    <!--end::Add user-->
...
<script>
    ...
    $("#product_type").on('click , change',function () {
        ...
        if (wahda != '') {
            $.get("{{ URL::to('products/add-sizes')}}" + '/' + wahda, function ($data) {
                $('#sizes').html($data);
            });
        }
    });
    $("#percentage").on('click , change',function () {
        $("#price_with_percentage").val(0);
        var percentage = $(this).val();
        var cost = $("#cost").val();

        var price_with_percentage = cost * percentage / 100;
        var pricePercentage = parseFloat(price_with_percentage) + parseFloat(cost);
        $("#price_with_percentage").val(pricePercentage);

    });
    $("#price").on('click , change',function () {
        var price = $(this).val();
        var cost = $("#cost").val();

        var final_price = ( parseFloat(price) - parseFloat(cost) ) / parseFloat(cost) * 100;
        $("#profit_percentage").val(final_price.toFixed(2));
