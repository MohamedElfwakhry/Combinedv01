...
@php
    $route = 'projects';
@endphp
@section('title',__('website.websiteProjects'))
@section('header')
    <!--begin::Heading-->
    @if(isset($id))
        <h1 class="text-dark fw-bolder my-0 fs-2">مشاريع المستخدم </h1>

    @else
        <h1 class="text-dark fw-bolder my-0 fs-2">{{trans('website.websiteProjects')}} </h1>
    @endif
    <!--end::Heading-->
    <!--begin::Breadcrumb-->
...
@section('script')
    ...
    <script type="text/javascript">
        $(function () {
            ...
            @if(isset($id))
            $.ajax({
                url: "{{ URL::to($route.'/add-button/'.$typee . '/'.$id)}}",
            ...
            @else
            $.ajax({
                url: "{{ URL::to($route.'/add-button/'.$typee)}}",
                success: function (data) {
                    $('.add_button').append(data);
                },
                dataType: 'html'
